...
def extract_info():
        ...
        "messages": [
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": """Read the image and extract all information in key value pairs and document type. Do not keep, Here’s the information extracted from the image in key-value pairs, in the begining:
 ...
 # Find the Document Type in the response content
        document_type = None
        ...
        if document_type:
            return jsonify(document_type, {"document_details": message_content})
        else:
            return jsonify({"error": "Document Type not found"}), 500
    else:
        return jsonify({"error": "Failed to extract information"}), 500
